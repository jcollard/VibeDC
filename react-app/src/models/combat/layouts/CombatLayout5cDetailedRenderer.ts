...
/**
 * Layout 5c: Detailed Top/Bottom Bars (variation)
 * - Top: Detailed info bar with more unit stats (taller)
 * - Middle: Map viewport
 * - Bottom: Larger combat log with more entries (taller)
 * Difference: Taller bars (48px top, 48px bottom) with more information
 */
export class CombatLayout5cDetailedRenderer implements CombatLayoutRenderer {
  private readonly TOP_BAR_HEIGHT = 48; // 4 tiles
  private readonly LOG_HEIGHT = 48; // 4 tiles
  ...
  private renderCombatLog(context: LayoutRenderContext): void {
    ...
    // Show last 3 entries
    const entriesToShow = combatLog.slice(-3);
