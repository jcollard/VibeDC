...
/**
 * Layout 1b: Compact Traditional RPG (variation)
 * - Left sidebar: Current + Target stacked (narrower)
 * - Center: Map viewport (wider)
 * - Right sidebar: Combat log + Turn order (narrower)
 * Difference: Narrower sidebars (72px vs 96px) for more map space
 */
export class CombatLayout1bCompactRenderer implements CombatLayoutRenderer {
  private readonly SIDEBAR_WIDTH = 72; // 6 tiles (narrower than original)
  ...
  private renderTurnOrderPanel(
    ...
    unitsToShow.forEach((unit, index) => {
      FontAtlasRenderer.renderText(
        ctx,
        `${index + 1}.${unit.name.substring(0, 7)}`,
        x + this.PANEL_PADDING + 6,
        currentY,
        fontId,
        fontAtlasImage,
