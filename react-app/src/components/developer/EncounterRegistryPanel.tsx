...
export const EncounterRegistryPanel: React.FC<EncounterRegistryPanelProps> = ({ onClose }) => {
  ...
  const handleGenerateEncounter = async () => {
    try {
      ...
      const selectedTilesetId = dungeonTileset?.id || (tilesets.length > 0 ? tilesets[0].id : 'forest');
      ...
      if (walkableTiles.length === 0) {
        alert('Failed to generate encounter: No walkable tiles found');
        return;
      }
      ...
      let encounterId = 'generated-encounter';
      let counter = 1;
      while (CombatEncounter.getById(encounterId)) {
        encounterId = `generated-encounter-${counter}`;
        counter++;
      }
      ...
      // Create the new encounter
      const newEncounter = CombatEncounter.fromJSON(encounterData as any);
      ...
      >
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <button
            ...
            title="Generate a random encounter with procedural dungeon"
          >
            Generate Encounter
          </button>
          <button
